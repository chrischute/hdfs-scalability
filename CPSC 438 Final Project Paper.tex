...
Some improvements have been suggested to solve the scalability problems with HDFS. Hadoop has provided the HDFS Federation, a solution which creates multiple NameNodes each of which has completely separate namespaces \cite{Federation}. This is not a solution to the underlying architectural problem, but rather essentially creates multiple systems and throws more hardware at the problem. This is still a potentially inefficient use of memory on the NameNodes and 
