...
However, there is a cost to having such large blocks. Mostly, this pushes the inefficiencies of the system onto the way disk of the DataNodes is allocated, instead of memory on the NameNode. Essentially, the very large virtual blocks will make the virtual memory and the mapping to physical memory more complicated on the DataNodes. Also, large blocks force the system to worry that appends that do not overrun a block will overrun the physical disk of the machine, so there will have to be careful safeguards in place. In practice HDFS has a large default block size of 128 MB. 

	Thus, we expect this memory limit to be effected in the following main ways. (1) Larger files will give the system a larger overall capacity. (2) Larger blocks will give the system larger overall capacity, assuming the limiting factor is memory on the NameNode and not disk on the DataNodes (since we can always add more DataNode machines). 
...
%TODO add sources here about federation, CalvinFS
